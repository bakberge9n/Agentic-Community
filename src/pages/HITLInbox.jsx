...
function PriorityBadge({ priority }) {
  ...
  return (
    <span className={cn("inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold uppercase tracking-wider border", config.bg, config.color)}>
      {config.label}
    </span>
  );
}
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-zinc-500 mt-1">
              Human-in-the-Loop approval queue — {pendingCount} pending {pendingCount === 1 ? "action" : "actions"}
            </p>
          </div>
          <div className="flex items-center gap-2">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search..."
                className="w-36 h-8 pl-8 pr-3 rounded-lg border border-zinc-200 bg-white text-xs text-zinc-900 placeholder:text-zinc-400 focus:outline-none focus:ring-1 focus:ring-zinc-300"
              />
            </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex gap-1 flex-shrink-0">
                      <button
                        ...
                        className="w-8 h-8 rounded-lg bg-emerald-50 border border-emerald-200 text-emerald-600 hover:bg-emerald-100 transition-colors flex items-center justify-center"
                        title="Approve"
                      >
                        <CheckCircle size={16} />
                      </button>
                      <button
                        ...
                        className="w-8 h-8 rounded-lg bg-red-50 border border-red-200 text-red-500 hover:bg-red-100 transition-colors flex items-center justify-center"
                        title="Reject"
                      >
                        <XCircle size={16} />
                      </button>
            ...
            >
              <div className="card sticky top-8">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="text-[10px] text-zinc-400 font-mono">{selected.id}</span>
                </div>
                ...
                <div className="space-y-3 mb-6">
                  ...
                  <div className="flex items-start gap-2 text-xs">
                    ...
                    <span className="text-amber-700">{selected.risk}</span>
                  </div>
                </div>
                ...
                <div className="space-y-2 text-xs text-zinc-500 mb-6">
                  <div className="flex justify-between">
                    <span>Agent</span>
                    <span className="text-zinc-700 font-medium">{selected.agent}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Type</span>
                    <span className="text-zinc-700 font-medium capitalize">{selected.type}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Received</span>
                    <span className="text-zinc-700 font-medium">{selected.timestamp}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Deadline</span>
                    <span className="text-amber-600 font-medium">{selected.deadline}</span>
                  </div>
                </div>
